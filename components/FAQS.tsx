  ...
  return (
    <div className="rc-accordion-card">
      <div className="rc-accordion-header">
        <div
          className={`rc-accordion-toggle p-4 bg-gray-900 border-b-2 border-gray-400  md:p-6 ${
            isActive ? "active" : ""
          }`}
          onClick={() => handleToggle(id)}
        >
          <h5 className={` ${Rancher} rc-accordion-title text-xl md:text-2xl `}>
            {id}. {header}
          </h5>
          {isActive ? (
            <svg
...
const FAQS: React.FC = () => {
  ...
  return (
    <div className="h-auto relative z-[111] bg-black px-[6vw] py-[45px] lg:py-[80px] flex flex-col justify-center items-start">
      <h2
        className={`${Rancher} text-4xl  md:text-left text-center md:text-7xl pb-8  text-white `}
      >
        Frequently Asked Questions
      </h2>
      <p
        className={`${SedaN} md:text-left text-center text-md md:text-xl text-white `}
      >
        Our FAQs section provides quick answers to common inquiries about our
        services, project timelines, support options, and more.
      </p>
